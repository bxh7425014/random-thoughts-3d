...
public class LoadTexImage_ARGB implements ILoadTexImage{
	private InputStream mCDTStream;
	private ByteArrayOutputStream mBytestream;
	public LoadTexImage_ARGB(InputStream stream) {
		mCDTStream = stream;
	}
	@Override
...
//			gl.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, GL11.GL_REPEAT);
			...
			gl.glTexImage2D(GL10.GL_TEXTURE_2D, 0, GL10.GL_RGBA, width, height, 0, GL10.GL_RGBA, GL10.GL_UNSIGNED_BYTE, ByteBuffer.wrap(imgdata));
			imgdata = null;
			try {
				mBytestream.close();
			} catch (IOException e) {
