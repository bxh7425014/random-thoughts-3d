...
import com.demo.utils.FloatUtils;
...
//	};
	...
	private short[] indices = Utils.INDICES;//new short[] { 0, 1, 2, 3 };
		...
		for (int index = 0; index < mTextures.length; index++) {
			...
			if (texture.getState() == Texture.STATE_LOADED) {
				...
				if (textureIndex > GL10.GL_TEXTURE31) {
					LogExt.Log(this, Thread.currentThread().getStackTrace(), "Error, textureIndex = " + textureIndex);
					return;
				}
